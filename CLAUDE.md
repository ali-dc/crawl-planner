...
- **`backend/`**: Python FastAPI application
  ...
  - `fetch_pubs.py`: Fetches the live pub list from the bristol.pub API into `data.json`
  - `parse.py`: Raw data parser (legacy, for `raw.data`)
...
- **`data/`**: Data directory (shared between backend and Docker)
  - `data.json`: Structured pub data (generated by `fetch_pubs.py`)
...
1. **Pub Data Fetching** (`fetch_pubs.py`) — current source of `data.json`:
   - Fetches the live pub list from `https://bristol.pub/api/pubs` (no auth or cookie required)
   - Normalizes it to the `data.json` schema (id, name, address, latitude, longitude)
   - Validates before writing: minimum pub count, no big drop vs the existing file, unique IDs, coordinates inside a Bristol bounding box
   - Only rewrites `data.json` when something changed; keeps a `data.json.bak` of the previous version
   - Run: `uv run fetch_pubs.py`, or hit `POST /api/refresh-pubs` (which also rebuilds the distance matrix)
   - Runs automatically every 30 days in the API (see "Monthly Pub Data Refresh" below)

   **Legacy** (`parse.py`): parses the obfuscated `raw.data` dump (fields like `_18`, `_20`). Kept for the
   `POST /api/parse` endpoint but superseded by `fetch_pubs.py`, which gets real address strings rather than
   the unresolved integer indices `parse.py` produces.
...
# Fetch the latest pub list from bristol.pub into data.json
uv run fetch_pubs.py

# Parse raw pub data (from raw.data) into structured format (legacy path)
uv run parse.py
...
# Pub data refresh
PUBS_API_URL=https://bristol.pub/api/pubs      # Upstream pub list endpoint
PUB_REFRESH_ENABLED=true                       # Set to false to disable the periodic refresh
PUB_REFRESH_INTERVAL_DAYS=30                   # Days between automatic refreshes
```
...
- `POST /api/refresh-pubs`: Fetch the latest pubs and rebuild distances (long-running; the usual way)
- `POST /api/parse`: Parse raw data into data.json (legacy)
- `POST /api/precompute`: Precompute distance matrix (long-running)
...
**Administration**:
- `POST /refresh-pubs`: Fetch the latest pub list and rebuild the distance matrix (long-running)
- `POST /precompute`: Trigger distance matrix precomputation (long-running, returns job status)
- `POST /parse`: Parse raw pub data from `raw.data` into `data.json` (legacy)

## Monthly Pub Data Refresh

The pub list is kept in sync with the upstream bristol.pub API. `fetch_pubs.py` does the fetching and
validation; `app.py` wires it to an endpoint and a background task.

### Bootstrap and Staleness Checks

`data/data.json` is **not** tracked in git — it is runtime state, rewritten in place by the refresh. On
startup (`_lifespan`) the app therefore:

1. Fetches the pub list from the API if `data.json` is missing, so a fresh deployment self-bootstraps
2. Compares the loaded matrix's `pub_ids` against `data.json`; on a mismatch it drops the planner and
   deletes `pub_distances.pkl`, so `init-precompute.sh` rebuilds it. `/plan` returns 503 until then —
   a mismatched matrix would map route indices onto the wrong pubs.

### Automatic Refresh

A background asyncio task (`PubCrawlPlannerApp._periodic_pub_refresh`) runs every
...
disable the task entirely.

### Manual Refresh

```bash
# On the server: fetch pubs + rebuild distances (takes several minutes)
curl -X POST http://localhost:8000/api/refresh-pubs

# Response
{
  "status": "success",
  "message": "Refreshed 425 pubs (42 added, 28 removed)",
  "pubs_count": 425,
  "previous_count": 411,
  "changed": true,
  "added": ["Beerd", "Bocabar", ...],
...
Returns 409 if a precomputation is already running, 502 if the upstream fetch or validation failed.
`data.json` is never overwritten when validation fails.

Or run the fetch alone (no distance rebuild) with `uv run fetch_pubs.py`; the distance matrix is then
stale until `precompute_distances.py` or `POST /api/precompute` runs.

### Validation Guards

`fetch_pubs.validate_pubs()` rejects an upstream response that would corrupt the dataset:
- Fewer than `MIN_EXPECTED_PUBS` (100) pubs
- A drop below `MAX_SHRINK_RATIO` (70%) of the current pub count
- Duplicate pub IDs
- Any coordinate outside `BRISTOL_BBOX`

### Effect on Shared Routes

`route_indices` in `shared_routes` are positions in the distance matrix, and those shift whenever the
pub list changes. `get_shared_route` therefore resolves pubs via the stored `selected_pub_ids`, falling
back to the index only for legacy rows that lack IDs. A pub that has since been delisted is dropped from
the response rather than replaced by whatever now occupies its old index; the stored leg geometry still
draws the original route.
